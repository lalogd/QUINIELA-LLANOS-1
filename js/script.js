        ...
        for (var i = 0; i < quantity; i++){
            if (results[i] && results[i] != undefined){
			    ...
			    for (var j =0; j < 10; j++) {
			  	...
			  	let cell2 = fila.insertCell(10);
			  	cell2.innerHTML += "<pre>" +results[i].split("\xa0\xa0")[10] + "</pre>";
			  	...
			  	if(results[i].split("\xa0\xa0")[10].length > 16)
			  		cell2.style.fontSize = "xx-small";
			  	else if(results[i].split("\xa0\xa0")[10].length > 11)
...
function updatedisplay(modo){
    ...
    if (modo == 1){
        ...
        if (aux>1){
    		var cellname = fila.insertCell(10);
...
function random(){
    ...
    for (var i = 0; i < 10; i++){
        ...
        partidos[i].getElementsByTagName("span")[r].style.backgroundColor = "#080808";
        res[i] = ["L","E","V"][r];
    }
    container.innerHTML = res.join("\xa0\xa0");
    costoactual();
...
function send(){ //Envía quiniela al whatsapp
    ...
    const quinbara = "*Quiniela de $0 pesos*%0A%0A";
    whatsapptext = encodeURI(localStorage.getItem("results"));
    whatsapptext = whatsapptext.split('*').join('%0D').replace(/#/g,"");
    window.location.href = "https://wa.me/525515372239?text="+quinbara + whatsapptext;}
}
