...
function selection(element){ 		//Pinta la casilla y actualiza el texto de la quiniela.
    ...
    if (!element.style.backgroundColor){
        ...
        element.style.backgroundColor = "rgb(65, 65, 6)";
    }
    else{
        if(combinations){
...
function allowcombination(){
    if (!combinations) 
    {
        ...
        document.getElementById("checkcombinaciones").style.backgroundColor =  "rgb(65, 65, 6)";
...
function random(){
    ...
    for (var i = 0; i < nPartidos; i++){
        ...
        partidos[i].getElementsByTagName("span")[r].style.backgroundColor =  "rgb(65, 65, 6)";
        res[i] = ["L","E","V"][r];
    }
    container.innerHTML = res.join("\xa0\xa0");
    costoactual();
...
setInterval(createConfetti, 150); // cantidad de confeti
animateConfetti();
